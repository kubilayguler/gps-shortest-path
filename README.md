...
# Start backend
cd backend
pm2 start npm --name "gps-backend" -- start

# Start frontend
cd frontend
pm2 start npm --name "gps-frontend" -- start
...
psql -U postgres -l | grep gps_shortest_path
...
- **Issues**: [GitHub Issues](https://github.com/kubilayguler/gps-shortest-path/issues)
...
- **Route Calculation**: < 2 seconds for 50+ stops
- **API Response Time**: < 100ms average
- **Database Queries**: Optimized with indexes
- **Frontend Load Time**: < 3 seconds initial load
- **Map Rendering**: Smooth 60 FPS interactions
